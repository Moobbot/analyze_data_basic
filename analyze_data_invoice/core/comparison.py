...
def compare_directories(output_file=config.DEFAULT_OUTPUT_DIFF):
    ...
    lines = []
    lines.append(
        "THỐNG KÊ KHÁC BIỆT FILE (So sánh theo tên file không tính đuôi mở rộng)"
    )
    lines.append("=" * 80)
    lines.append(
        f"Tổng số file gốc (Dataset): {sum(len(v) for v in dataset_files.values())} file ({len(dataset_bases)} tên duy nhất)"
    )
    lines.append(
        f"Tổng số file nhãn (Label): {sum(len(v) for v in label_files.values())} file ({len(label_bases)} tên duy nhất)"
    )
    lines.append("-" * 80)
    ...
    if missing_in_label:
        ...
        for base in sorted(missing_in_label):
            ...
            for f in dataset_files[base]:
                full_path = config.DATASET_DIR / f
