...
def compare_libs():
    ...
    with open(
        os.path.join(output_dir, "output_pypdf2.txt"), "w", encoding="utf-8"
    ) as f:
        f.write(text_pypdf2)
    ...
    with open(
        os.path.join(output_dir, "output_pymupdf.txt"), "w", encoding="utf-8"
    ) as f:
        f.write(text_fitz)
    ...
    with open(
        os.path.join(output_dir, "output_pdfplumber.txt"), "w", encoding="utf-8"
    ) as f:
