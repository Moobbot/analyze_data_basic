...
**Mô tả**: Dữ liệu hóa đơn từ các nhà cung cấp.
**File nguồn**: [label_schema.py](file:///d:/Work/Clients/AIRC/product/ACPA/analyze_data_basic/analyze_data_invoice/lib/label_schema.py)

| Tên trường (Field) | Kiểu (Type) | Bắt buộc | Quy tắc xử lý / Validation                                | Mô tả                            |
| :----------------- | :---------- | :------: | :-------------------------------------------------------- | :------------------------------- |
| **Type**           | Text        |    Có    | - Validate chuỗi ký tự.                                   | Loại hóa đơn.                    |
| **No**             | Text        |    Có    | - Validate chuỗi ký tự.<br>- Có thể Null.                 | Số hóa đơn.                      |
| **Date**           | Date        |    Có    | - Định dạng: `MM/DD/YYYY` (Regex: `^\d{2}/\d{2}/\d{4}$`). | Ngày hóa đơn.                    |
| **Customer**       | Text        |  Không   | - Validate chuỗi ký tự.                                   | Khách hàng.                      |
| **Supplier**       | Text        |  Không   | - Validate chuỗi ký tự.                                   | Nhà cung cấp.                    |
| **Currency**       | Text        |  Không   | - Validate chuỗi ký tự.                                   | Loại tiền tệ.                    |
| **Ex rate**        | Number      |  Không   | - Validate kiểu số (int/float).                           | Tỷ giá.                          |
| **Ex rate to SGD** | Number      |  Không   | - Validate kiểu số (int/float).                           | Tỷ giá sang SGD.                 |
| **Description**    | Array       |    Có    | - Danh sách chi tiết hóa đơn.                             | Mảng chứa các hạng mục chi tiết. |

### Chi tiết bản ghi trong "Description":

| Tên trường (Field)          | Kiểu (Type) | Bắt buộc | Quy tắc xử lý / Validation | Mô tả                         |
| :-------------------------- | :---------- | :------: | :------------------------- | :---------------------------- |
| **text**                    | Text        |  Không   | - Validate chuỗi ký tự.    | Nội dung chi tiết.            |
| **Amount (before tax)**     | Number      |  Không   | - Validate kiểu số.        | Số tiền trước thuế.           |
| **Tax amount**              | Number      |  Không   | - Validate kiểu số.        | Tiền thuế.                    |
| **Amount (after GST)**      | Number      |  Không   | - Validate kiểu số.        | Số tiền sau thuế GST.         |
| **Amount in SGD**           | Number      |  Không   | - Validate kiểu số.        | Số tiền quy đổi SGD.          |
| **Tax amount in SGD**       | Number      |  Không   | - Validate kiểu số.        | Tiền thuế quy đổi SGD.        |
| **Amount after tax in SGD** | Number      |  Không   | - Validate kiểu số.        | Số tiền sau thuế quy đổi SGD. |
| **Project code**            | Text        |  Không   | - Validate chuỗi ký tự.    | Mã dự án.                     |
| **Tax type**                | Text        |  Không   | - Validate chuỗi ký tự.    | Loại thuế.                    |
